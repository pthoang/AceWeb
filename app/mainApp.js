...
angular.module('mainApp', [
    ...
    .controller('mainController', function($scope, $window,$location, $http, $cookies) {
        var todayDate = new $window.Date();
        $scope.expDate = new $window.Date(todayDate.getFullYear() + 1, todayDate.getMonth(), todayDate.getDate());
        ...
        $http.post('/api', '')
            .success(function (response) {
                ...
                if(!$cookies.getObject('userId')) {
                    $http.get($scope.url + '/anonymous')
                        .success(function (response) {
                            $scope.showAll = true;
                            $cookies.putObject('userId', response.uuid, {expires: $scope.expDate})
                        })
                } else {
                    $cookies.putObject('userId', $cookies.getObject('userId'), {expires: $scope.expDate});
                    $scope.showAll = true;
                }
            });
        ...
        $scope.mobileNav = function () {
            if((window.mobilecheck()) || window.innerWidth < 960) {
                return false;
            }
            return true;
        };
        ...
        $scope.setColor = function (color) {
            ...
            return {"background-color": '#283845'};
        };
